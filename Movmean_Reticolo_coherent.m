function Comp=Movmean_Reticolo_coherent(My_Param,value,ref)
...
tic;
NN=rand(1,length(My_Param.material.list));
for k=1:length(My_Param.material.list)
    a{k}=@(ld) Import_Material(My_Param.material.list(k),ld, 0,0);
end

retabeles(NN,a{:});

[r_TE,t_TE]=retabeles(0,[1,NN,1],My_Param.material.thickness,0,2*pi./My_Param.wave.Lam);
[r_TM,t_TM]=retabeles(2,[1,NN,1],My_Param.material.thickness,0,2*pi./My_Param.wave.Lam);
...
% Movmean
Mov_Rup=movmean(Rup,value);
Mov_Tup=movmean(Tup,value);
Mov_Abs=movmean(Abs,value);

Rel_diff_Abs=abs(Mov_Abs-ref.Abs0')./ref.Abs0'*100;
Rel_diff_Refl=abs(Mov_Rup-ref.Rup0')./ref.Rup0'*100;
Max_diff_Refl=max(Rel_diff_Refl,Rel_diff_Abs);
Comp=sum(Max_diff_Refl);
% toc
